...
public class RoomService {
  ...
  public Room createRoom(
    ...
    // check room number
    if (!inputIsValid(roomNum)) throw new IllegalArgumentException(
    ...
    if (
      roomRepository.findRoomByRoomNum(roomNum) != null
  ...
  @Transactional
  public Room updateRoom(String currentUserId, String roomNum, int newCapacity)
    throws Exception {
    // check capacity
    if (newCapacity < 1) throw new IllegalArgumentException(
      "Capacity must be at least 1"
    );
    ...
    if (toUpdate == null) throw new IllegalArgumentException(
  ...
  public Room deleteRoom(String currentUserId, String roomNum)
    ...
    if (toDelete.getRoomBookings() != null) {
      for (RoomBooking rb : toList(toDelete.getRoomBookings())) {
        if (
          rb.getDate().equals(Date.valueOf(LocalDate.now())) &&
          rb.getEndTime().after(Time.valueOf(LocalTime.now())) ||
          rb.getDate().after(Date.valueOf(LocalDate.now()))
        ) throw new IllegalArgumentException(
